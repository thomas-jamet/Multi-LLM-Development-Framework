...
def fetch_skill(source: str):
    ...
    if source.startswith("http"):
        ...
        url = GITHUB_RAW_BASE.format(owner=owner, repo=repo, branch=branch, path=path)
    else:
        print(
            "❌ Invalid source format. Use 'owner/repo/path/to/skill.md' or full URL."
        )
        sys.exit(1)
...
def main():
    ...
    fetch_parser.add_argument(
        "source", help="GitHub shorthand (owner/repo/path) or URL"
    )
