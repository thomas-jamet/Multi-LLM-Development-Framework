...
def extract_description(content: str) -> str:
    ...
    return (
        " ".join(desc_lines)[:150] + "..." if desc_lines else "No description available"
    )
...
def install_skill(skill: dict):
    ...
    if target_file.exists():
        overwrite = (
            input(f"⚠️  {filename} already exists. Overwrite? [y/N]: ").strip().lower()
        )
        if overwrite != "y":
...
def main():
    ...
    parser.add_argument("query", help="Search term (try 'all', 'bash', 'sql', etc.)")
